    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-2.5 bg-crayon-yellow/40 border-b-[3px] border-ink shrink-0">
        <div className="max-w-5xl mx-auto flex items-center gap-2 sm:gap-4">
          <span className="text-sm sm:text-base font-body font-bold text-ink shrink-0">Your top 3:</span>
                ...
                <div key={r} className="flex items-center gap-1.5 min-w-0">
                  ...
                  {p ? (
                    <div className="flex items-center gap-1.5 min-w-0">
                      <img src={p.imageData} alt="" className="w-9 h-9 rounded-md border-2 border-ink bg-white object-contain shrink-0" />
                      <span className="text-sm font-body font-semibold text-ink/80 truncate hidden sm:block">{p.employee.name}</span>
                    </div>
                  ) : (
                    <span className="text-sm font-body font-semibold text-ink/50 italic">not picked</span>
                  )}
                </div>
              );
            })}
        ...
        <p className="text-xs sm:text-sm text-ink/70 font-body font-medium mt-1 text-center">Tap a medal to give it to a doodle (it moves here from wherever it was). Tap the same medal again to remove it. Your picks always show above.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {doodles.length === 0 ? (
          <div className="text-center py-16 space-y-2">
            ...
            <p className="text-ink/60 font-body font-semibold text-lg">No doodles yet. Be the first!</p>
          </div>
        ) : (
              ...
              <div key={d.id} className="card overflow-hidden flex flex-col">
                <div className="flex items-center gap-2.5 px-3 pt-2.5 pb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-base truncate text-ink leading-tight">{d.employee.name}</div>
                    <div className="text-xs text-ink/60 font-semibold truncate">{d.employee.department}</div>
                  </div>
                ...
                <div className="px-3 py-2.5 space-y-2 mt-auto">
                  <p className="text-sm text-ink/70 italic font-body font-medium truncate">&quot;{d.prompt}&quot;</p>
                  {d.isOwn ? (
                    ...
                    <div className="flex gap-1.5">
                      {[1, 2, 3].map((rank) => (
                        <button
                          ...
                          className={`flex-1 py-2 rounded-xl text-sm font-body font-bold transition-all border-2 border-ink disabled:opacity-50 ${
                            d.myVoteRank === rank
